...
import { Image, Text, useSend, useMention, Mention } from 'alemonjs'
...
if (!existsSync(marryDataPath)) writeFileSync(marryDataPath, JSON.stringify({ ren: [], renmin: [] }), 'utf-8');
...
export default OnResponse(async (event, next) => {
    ...
    if (/亲密排行(.*)/.test(event.MessageText)) {
        ...
        let nickList = idList.map(item => (renmin.slice().reverse())[item.id as number])
