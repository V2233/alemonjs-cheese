...
export default OnResponse(async (event, next) => {
    ...
    // 被At的人
    let user = Mentions.find(item=>!item.IsBot)
    ...
    if (/搜索图片(.*)/.test(event.MessageText)) {
        ...
        for (const info of pngsList) {
            pics.push({ url: info.largeUrl, id: index, tag: info.tagList, detail: info.detailInfo })
            index++
        }
    ...
    if (match = event.MessageText.match(/合成头像(\d+)/)) {
        ...
        const img = await Pictures('makeEmo', {
            data: {
                originUrl: await (user?user:event).UserAvatar?.toURL() || `https://q1.qlogo.cn/g?b=qq&s=0&nk=${event.UserId}`,
                maskUrl: url,
                mixBlendMode
            }
        })
