...
import type { ICache, IGroup, IGengItem, ICdCache, ICdTip } from '@src/types/meme'
...
let gengList: IGengItem[] = JSON.parse(readFileSync(join(pluginInfo.PUBLIC_PATH, 'apps', 'geng', 'geng.json'), 'utf8')) || []
let cache: ICache = {}
let page = 1
    ...
    if (/看图识梗|结束|懂王排行|识梗难度设置(.*)/.test(event.MessageText) || cdCache[event.GuildId]) {
        ...
        if (cdCache[event.GuildId]) {
            ...
            if (String(groupData().ans) == playerAns) {
                ...
                Pictures('memeQs', {
                    data: {
                        avatar: await event.UserAvatar?.toURL() || '',
