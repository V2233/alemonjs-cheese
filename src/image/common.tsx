...
export function HeaderBox(data: IHeaderBoxProps) {
    ...
    return (cfg.header_visible &&
        <div className='head_box' style={style}>
            <div className="id_text" style={data.titleStyle}>{data.title}</div>
...
export function DataBox({children,style}:{children:React.ReactNode,style?:React.CSSProperties}) {
    const cfg = Cfg.getConfig('theme')
    return (
        <div className='data_box' style={
            {
...
export function Item({children,color,classname,style}:{children:React.ReactNode,color?:string,classname:'item' | 'itemOne',style?:React.CSSProperties}) {
    const cfg = Cfg.getConfig('theme')
...
export function PageLable({ text }: { text: string }) {
    const cfg = Cfg.getConfig('theme')
    return (
        <div className="page_lable" style={{ background: `linear-gradient(90deg, ${hexToRgb(cfg.mask_color,cfg.mask_opacity)}, rgba(194, 194, 194, ${cfg.mask_opacity}))` }}>{text}</div>
    )
}
...
export function Copyright({text}:{text?:string}) {
    const cfg = Cfg.getConfig('theme')
    return (
        <div className="logo" style={{ color: '#' + cfg.mask_color }}>{text?text:`${botInfo.BOT_NAME} ${botInfo.BOT_VERSION} & ${pluginInfo.PLUGIN_NAME} ${pluginInfo.PLUGIN_VERSION}`}</div>
    )
}
...
export function Template(data: ITemplate): React.ReactNode {
    const cfg = Cfg.getConfig('theme')
    return (
        <html lang="zh-CN">
            <head>
