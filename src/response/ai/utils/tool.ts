...
import { Image, Mention, type PublicEventMessageCreate, Text, useSend } from 'alemonjs';
...
export default class AiTool {
  ...
  async getRoleText(text: string, recentMsgs) {
    ...
    const memberTip =
      Object.keys(memberMap).length == 0
        ? ''
        : '你接下来可能会用到的部分成员QQ和昵称的字典是：' + JSON.stringify(memberMap) + '。';
  ...
  async send(parts) {
    ...
    try {
      ...
      return await send(
        Image(await this.toMarkdown(`## ${err.message}\n\n\`\`\`bash\n${err.stack}\n\`\`\``))
      );
    }
  }
}
