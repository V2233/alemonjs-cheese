...
if (!existsSync(luckDataPath))
  writeFileSync(luckDataPath, JSON.stringify({}), "utf-8");

let luckRecord: ILuckRecord =
  JSON.parse(readFileSync(luckDataPath, "utf8")) || {};
...
scheduleTask(
  () => {
    ...
    console.warn(
      `[奶酪刷新运势状态]：如定时刷新失败管理员可发送【刷新运势】进行刷新！`
    );
  },
  {
    hour: 0,
    ...
    if (/逆天改命/.test(event.MessageText)) {
      ...
      if (fortuneList[getTodayLuck().id].stars >= 7) {
        if (playerObj().isKing) {
          ...
          Send(
            Text(
              "不用改命啦，至尊无敌运气王，您已经施展了足以威慑众生的运势权能！"
            )
          );
        }
        return;
      ...
      if (luckRecord[strUser].debris < 7) {
        Send(
          Text(`你现在只有${playerObj().debris}个碎片哦~集满7个碎片再来吧！`)
        );
        return;
      }
      luckList = fortuneList.filter(
        (item) =>
          item.stars >
          (fortuneList[getTodayLuck().id].stars < 5
            ? fortuneList[getTodayLuck().id].stars
            : 6)
      );
