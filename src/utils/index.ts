 ...
 */
export function getTime(timeStamp = new Date(), format = 'YYYY/MM/DD hh:mm:ss') {
 ...
 * @returns 
 */
export function hexToRgb(hex:string, opacity = 0.5) {
    hex = hex.replace(/^#/, '');
    if (hex.length === 3) {
 ...
 * @returns 
 */
export async function createMD5(buffer:Buffer) {
