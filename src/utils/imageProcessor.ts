import { readdirSync, statSync, mkdirSync, createReadStream, writeFile, copyFileSync } from 'fs'
...
// 递归处理文件夹
export function processFolder(inputFolder, outputFolder) {
...
// 压缩图片
export function compressImage(inputFilePath, outputFilePath, targetSize = 0): Promise<Buffer | boolean> {
 ...
 * @returns 
 */
export function compressImageFromBuffer(buffer: Buffer, targetSize = 0): Promise<Buffer> {
    const theme = Cfg.getConfig('theme')
    if (!targetSize) targetSize = theme?.compress || 0
    if (targetSize === 0) {
...
export function dealWithStream(basePicture) {
    ...
    streamToBuffer(transformer).then(function (newPicBuffer: Buffer) {
        //写入新图片中
...
export function dealWithBuffer(basePicture) {
    sharp(basePicture)
        .resize(800, 600, {
...
//添加文字水印
export function addText(basePicture, font, newFilePath) {
...
//添加图片水印,水印图片一定要小于原图
export function addWatermark(basePicture, watermarkPicture, newFilePath) {
    sharp(basePicture)
        //.rotate(180) //旋转180度
        .composite([{
