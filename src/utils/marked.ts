...
// let run: typeof import('@mermaid-js/mermaid-cli').run
// try {
//     run = (await import('@mermaid-js/mermaid-cli')).run
// } catch(err:any) {
//     logger.warn(`[cheese]依赖(@mermaid-js/mermaid-cli)加载失败:`, err.stack ??err.message)
// }
...
export async function toMermaid<T extends keyof IMermaidFormat>(
  text: string,
  outFormat?: T
): Promise<IMermaidFormat[T]> {
  //   if (!/```mermaid/.test(text)) text = mermaid2Md(text);

  //   const inputPath = join(pluginInfo.DATA_PATH, 'mermaid.md');
  //   const outputPath = join(pluginInfo.DATA_PATH, `mermaid.${outFormat ? outFormat : 'svg'}`) as
  //     | `${string}.png`
  //     | `${string}.svg`
  //     | `${string}.pdf`
  //     | `${string}.md`
  ...
  if (outFormat == 'svg') {
    ...
    const { data, width, height } = await renderPng(text, { scale: 2, background: 'white' });
    return data;
  }
}
