...
var res_default = OnResponse(async (event, next) => {
	...
	if (cdTip[event.GuildId] && !cdCache[event.GuildId]) {
		...
		sendAtText(`已超时结束，请重新发起【看图识梗】！\npass: 当前设置为 ${cache[event.GuildId].cd} 秒超时结束！\n`, {
			md: (fmd) => fmd.addText(`当前难度等级${qsDegree()}，发送\n【识梗难度设置 `).addButton("简单", { data: "识梗难度设置简单" }).addText(" | ").addButton("一般", { data: "识梗难度设置一般" }).addText(" | ").addButton("困难", { data: "识梗难度设置困难" }).addText(" | ").addButton("地狱", { data: "识梗难度设置地狱" }).addText(`】(答对分别加 1 | 2 | 3 | 4 分)`),
