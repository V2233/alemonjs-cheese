...
var GroupStore = class {
	...
	async getGroup(group_id, platform) {
		...
		switch (platform) {
			case "qq": {
				const qqGroupMap = await (await global.client.pickGroup(Number(group_id))).getMemberMap();
				for (const [user_id, user] of qqGroupMap) group_map[user_id] = {
					user_id,
			...
			case "one-bot": {
				...
				for (const member of onebotGroup.data) group_map[member.user_id] = {
					user_id: member.user_id,
					nickname: member.nickname,
					avatar: member.avatar ? member.avatar : `http://q2.qlogo.cn/headimg_dl?dst_uin=${member.user_id}&spec=5`
				};
				this.map[group_id] = {
					group_id,
					group_map
			...
			default: return this.map[group_id] || {
				group_id,
				group_map
			};
		}
...
var res_default = OnResponse(async (event, next) => {
	...
	if (event.Platform != "qq" && event.Platform != "onebot") {
		...
		if (!isReaded) {
			if (existsSync(groupDataPath)) groupStore.map = JSON.parse(readFileSync(groupDataPath, "utf-8"));
			isReaded = true;
		}
		if (!groupStore.map[event.GuildId]) groupStore.map[event.GuildId] = {
...
//#endregion
export { res_default as default, groupStore };