...
import { getRandomFaces, faceMap } from './face.js';
...
class AiTool {
    ...
    async getRoleText(text, recentMsgs) {
        ...
        const memberTip = Object.keys(memberMap).length == 0
            ? ""
            : "你接下来可能会用到的部分成员QQ和昵称的字典是：" +
                JSON.stringify(memberMap) +
                "。";
                ...
                switch (seg.type) {
                    ...
                    case "send_like": {
                        ...
                        switch (this.platform) {
                            case "qq":
                                await this.client
                                    .pickMember(Number(this.e.GuildId), Number(user_id))
                                    .thumbUp(Number(times) || 1);
                                break;
                            case "onebot":
                    ...
                    case "send_poke": {
                        switch (this.platform) {
                            case "qq":
                                await this.client
                                    .pickMember(Number(this.e.GuildId), Number(this.e.UserId))
                                    .poke();
                                break;
                            case "onebot":
                                await this.client.sendApi({
                    ...
                    case "set_group_name": {
                        switch (this.platform) {
                            case "qq":
                                await this.client
                                    .pickGroup(Number(this.e.GuildId))
                                    .setName(seg.data);
                    ...
                    case "set_group_card": {
                        ...
                        switch (this.platform) {
                            case "qq":
                                await this.client
                                    .pickGroup(Number(this.e.GuildId))
                                    .setCard(Number(user_id), newMemberCard);
                                ret.push(Text("改好啦，你看看有没有生效"));
                                break;
                            case "onebot":
                                await this.client.sendApi({
