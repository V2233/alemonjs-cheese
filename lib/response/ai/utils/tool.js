...
import { faceMap, getRandomFaces } from "./face.js";
import { Image, Mention, Text, useSend } from "alemonjs";

//#region src/response/ai/utils/tool.ts
var AiTool = class {
	e;
	platform = "qq";
	constructor(e) {
	...
	/** 获取角色 */
	async getRoleText(text, recentMsgs) {
	...
	async getMemberMapFromMsg() {
		...
		if (!groupMemberMap) return void 0;
		let memberMap = {};
		...
		for (let [user_id, info] of Object.entries(groupMemberMap)) {
			...
			for (let word of String(info.nickname || "")) if (msgSet.has(word) && word !== "") {
				memberMap[user_id] = {
					昵称: info.nickname,
					头像: info.avatar
				};
		...
		return Object.keys(memberMap).length == 0 ? void 0 : memberMap;
	}
	/** 解析ai回答文本 */
	parse(input) {
		const result = [];
		...
		while (i < input.length) {
			...
			for (const { type, pattern } of tagPatterns) {
				...
				if (match) {
					matched = true;
					if (match.index > i) result.push({
						type: "text",
						text: input.slice(i, match.index).trim()
					});
					result.push({
						type,
						data: match[1].trim()
	...
	/** 文本转markdown */
	async toMarkdown(text) {
		const img = await Pictures("markdown", { data: {
			html: await toMarkdown(text),
			avatar: this.e.UserAvatar || ""
		} });
		if (typeof img != "boolean") return img;
		else throw new Error("图片加载失败");
	}
	/** 文本转html */
	async toHtml(text) {
	...
	async toMermaid(text) {
		...
		} else return await toMermaid(text, "png");
	}
	/** 文字转语音 */
	async toTTS(text, id = 199) {
	...
	async send(parts) {
		...
		try {
			for (let seg of parts) switch (seg.type) {
				case "text":
				...
				case "send_like": {
					...
					switch (this.platform) {
						...
						case "onebot":
							...
							break;
						default: ret.push(Text("该平台不支持戳一戳哦~"));
					}
					break;
				}
				case "send_poke":
				...
				case "set_group_name":
					switch (this.platform) {
						...
						case "onebot":
							...
							ret.push(Text("改好啦，你看看有没有生效"));
							break;
						default: ret.push(Text("该平台不支持修改群名哦~"));
					}
					break;
				case "set_group_card": {
					...
					switch (this.platform) {
						...
						case "onebot":
							...
							ret.push(Text("改好啦，你看看有没有生效"));
							break;
						default: ret.push(Text("该平台不支持修改群昵称哦~"));
					}
					break;
				}
				case "set_group_special_title": {
					...
					switch (this.platform) {
						...
						case "onebot":
							...
							ret.push(Text("改好啦，你看看有没有生效"));
							break;
						default: ret.push(Text("该平台不支持修改群头衔哦~"));
					}
					break;
				}
				case "set_group_ban": {
					...
					switch (this.platform) {
						...
						case "onebot":
							...
							break;
						default: ret.push(Text("该平台不支持禁言哦~"));
					}
					break;
				}
				case "set_group_whole_ban": switch (this.platform) {
					case "qq":
			...
			if (ret.length > 0) return await send(...ret);
		} catch (err) {
