...
import { useSend, useMention, ResultCode, Text, Mention, Image } from 'alemonjs';
...
if (!existsSync(marryDataPath))
    writeFileSync(marryDataPath, JSON.stringify({ ren: [], renmin: [] }), "utf-8");
...
var res = OnResponse(async (event, next) => {
    ...
    if (/亲密排行(.*)/.test(event.MessageText)) {
        ...
        if (typeof img != "boolean") {
            await Send(Mention(event.UserId), Image(img), Text(`您还可以发送【亲密排行2】查看其他情侣~`));
        }
        else {
            Send(Text("图片加载失败"));
        }
    ...
    if (/^(\/|#)?抢群友(.*)/.test(event.MessageText)) {
        ...
        let lm = { man: event.UserName, woman: name };
        ren.push(dx);
        renmin.push(lm);
    ...
    if (/我对象呢|今日老婆/.test(event.MessageText)) {
        ...
        if (i1 > 0) {
            let a = "http://q2.qlogo.cn/headimg_dl?dst_uin=" +
                ren[i1 - 1].woman +
                "&spec=5";
    ...
    if (/老婆亲亲|老婆羞羞|老婆打你|老婆逛街|老婆做饭|亲密排行(.*)/.test(event.MessageText)) {
        ...
        if (currentId == -1) {
            ...
            if (favorCd[event.UserId]) {
                ...
                Send(Mention(event.UserId), Text(`你还有${favorCd[event.UserId].cd - Math.floor(currentTime / 1000)}秒cd来提升好感,基础cd为20s，最长为2分钟，好感度越高，cd越长哦~\npass: 🚫禁止恶意重复刷指令，一经发现拉黑处理!`));
                return;
            }
            ...
            //恩爱列表
            let favorList = `\n【老婆亲亲】  亲密度：0.2~0.5 ↑ \n【老婆羞羞】  亲密度：0.4~0.7 ↑ \n【老婆逛街】  亲密度：0.6~0.9 ↑ \n【老婆做饭】  亲密度：0~0.5 ↓ \n【老婆打你】  亲密度：清0`;
            let partner = ren[currentId].woman == event.UserId
                ? ren[currentId].man
                : ren[currentId].woman;
    ...
    writeFileSync(marryDataPath, JSON.stringify({ ren: ren, renmin: renmin }), "utf-8");
}, "message.create");
// 亲密值排行
