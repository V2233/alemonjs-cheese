...
import { assetsPath, port } from "../../utils/server.js";
...
const getHybridTip = () => {
	return mixMode.map((item, index) => `${index + 1}. **${item.desc}**\`\`\`${item.name}\`\`\`：${item.detail}`).join("\n");
};
var res_default = OnResponse(async (event, next) => {
	...
	if (/搜索图片(.*)/.test(event.MessageText)) {
		...
		const img = await Pictures("emoList", { data: {
			list: pics,
			pageNo
		} });
		if (typeof img != "boolean") Send(Image(img));
	...
	if (match = event.MessageText.match(/获取图片(\d+)/)) {
		...
		if (id < 0 || id >= pageSize) Send(Text(`发送0~14`));
	...
	if (match = event.MessageText.match(/合成头像(\d+)/)) {
		...
		if (!existsSync(maskPngPath)) writeFileSync(maskPngPath, maskBuffer, "utf-8");
		url = `http://127.0.0.1:${port}/cache/${maskMd5}.png`;
		const img = await Pictures("makeEmo", { data: {
			originUrl: (user ? user : event).UserAvatar || `https://q1.qlogo.cn/g?b=qq&s=0&nk=${event.UserId}`,
			maskUrl: url,
			mixBlendMode
		} });
		if (typeof img != "boolean") Send(Image(img));
	...
	if (/^第(\d+)页$/.test(event.MessageText)) pageNo = (Number((/第(\d+)页/.exec(event.MessageText) || [])[1]) || 1) - 1;
}, "message.create");
