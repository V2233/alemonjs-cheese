	...
	if (/^(\/|#)?(今日运势|运气|祝福|诅咒|逆天改命)$/.test(event.MessageText)) {
		...
		if (/逆天改命/.test(event.MessageText)) {
			if (!playerObj() || playerObj().list.length == 0) {
				sendAtText("你还没看今天的运势呢，改什么命啊o(≧▽≦o)", { btns: (fbg) => fbg.addRow().addButton("今日运势", "今日运势", { autoEnter: true }) });
				return;
			}
			if (atUser) {
				sendAtText("哼~你还想帮别人改命？", { btns: (fbg) => fbg.addRow().addButton("逆天改命", "逆天改命", { autoEnter: true }) });
				return;
			}
			if (fortuneList[getTodayLuck().id].stars >= 7) {
				if (playerObj().isKing) sendAtText("不用改命啦，至尊无敌运气王，发起【诅咒@群友】或【祝福@群友】展示您的运势权能吧！\n", { md: (fmd) => fmd.addButton("祝福@群友", { data: "祝福" }).addText("  ").addButton("诅咒@群友", { data: "诅咒" }) });
				else sendAtText("不用改命啦，至尊无敌运气王，您已经施展了足以威慑众生的运势权能！");
				return;
			}
		...
		if (/诅咒/.test(event.MessageText)) {
			if (!luckRecord[event.UserId].isKing) {
				sendAtText("您没有干扰别人运势的权能或者您已经改变过别人的命运了，成为至尊无敌运气王才可以获得一次干涉他人命运的权利哦~");
				return;
			}
			if (!atUser) {
		...
		if (/祝福/.test(event.MessageText)) {
			...
			if (atUser.UserId == event.UserId) {
				sendAtText("不用祝福自己哦~你已经是运气王了！", { btns: (fbg) => fbg.addRow().addButton("诅咒", "诅咒").addButton("祝福", "祝福") });
				return;
			}
			if (!luckRecord[atUser.UserId] || !luckRecord[atUser.UserId].isTested) {
		...
		if (!playerObj().isTested || /逆天改命/.test(event.MessageText)) {
			luckId = Math.floor(Math.random() * luckList.length);
			if (luckList[luckId].stars == 0) addDebris = 0;
		...
		let mixText = ``;
		if (starCount == 7) {
			if (playerObj().isKing) mixText = `恭喜你成为了至尊无敌运气王！将笼罩在命运之神欢愉的曙光下！你有一次决定别人命途的权利，请使用【诅咒@群友】或【祝福@群友】施展您的运势权能吧！`;
