	...
	*/
	mergeYamlFile() {
		...
		for (const file of files) {
			...
			if (!existsSync(cfgFile)) copyFileSync(cfgFileDef, cfgFile);
			else {
				const cfg = this.getConfig(file.replace(".yaml", "").replace(/.*\\|.*\//g, ""));
				const doc = new YamlHandler(cfgFileDef);
				const defCfg = doc.jsonData;
				const cfgKeys = Object.keys(cfg);
				doc.yamlPath = cfgFile;
				cfgKeys.forEach((key) => {
					if (!defCfg.hasOwnProperty(key)) delete cfg[key];
				});
				Object.entries(defCfg).forEach(([key, value]) => {
					if (key in cfg) {
						if (value instanceof Object) doc.set(key, Object.assign(value, cfg[key]));
						else doc.set(key, cfg[key]);
					} else doc.set(key, value);
				});
				if (file == "help") doc.set("default", defCfg["default"]);
				doc.save(cfgFile);
			}
		}
	}
